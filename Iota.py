...
def rolling_oos_analysis(daily_ret: pd.Series, oos_start_dt: date, 
    ...
    """Rolling analysis to calculate iota values over time windows."""
        ...
        with st.form("analysis_form"):
            ...
            with col1:
                enable_rolling = st.checkbox(
                    ...
                    value=True,
                    help="Analyze iota values over rolling time windows"
                )
           ...
           try:
               ...
               if config['enable_rolling']:
                   ...
                   if rolling_results.get('sufficient_data', False):
                       st.success("✅ Rolling analysis complete! Check the 'Rolling Analysis' tab for detailed insights.")
                   else:
                       st.warning("⚠️ Rolling analysis completed but insufficient data for meaningful results.")
               else:
                   st.info("ℹ️ Rolling analysis disabled. Enable it in the configuration to see performance over time.")
...
# Rolling Analysis Tab
    with tab3:
        ...
        if hasattr(st.session_state, 'rolling_results') and st.session_state.rolling_results:
            ...
            # Display rolling analysis summary
            st.subheader("📊 Rolling Analysis Summary")
            ...
            if rolling_results.get('sufficient_data', False):
                ...
                with col3:
                    st.metric("Step Size", f"{rolling_results['step_size_days']} days")
                ...
                st.subheader("📊 Rolling Iota Statistics")
                
                if 'metric_iotas' in rolling_results:
                    metric_names = {'sh': 'Sharpe Ratio', 'cr': 'Cumulative Return', 'so': 'Sortino Ratio'}
                    
                    for metric_key, metric_name in metric_names.items():
                        if metric_key in rolling_results['metric_iotas']:
                            iotas = rolling_results['metric_iotas'][metric_key]
                            if len(iotas) > 0:
                                with st.expander(f"📊 {metric_name} Rolling Statistics"):
                                    col1, col2, col3, col4 = st.columns(4)
                                    with col1:
                                        st.metric("Mean Iota", f"{np.mean(iotas):+.3f}")
                                    with col2:
                                        st.metric("Median Iota", f"{np.median(iotas):+.3f}")
                                    with col3:
                                        st.metric("Std Dev", f"{np.std(iotas):.3f}")
                                    with col4:
                                        st.metric("Valid Windows", f"{len(iotas)}")
                                    
                                    # Show distribution
                                    positive_pct = (iotas > 0).mean() * 100
                                    negative_pct = (iotas < 0).mean() * 100
                                    neutral_pct = 100 - positive_pct - negative_pct
                                    
                                    st.write(f"**Performance Distribution:**")
                                    st.write(f"- Positive (ι > 0): {positive_pct:.1f}%")
                                    st.write(f"- Negative (ι < 0): {negative_pct:.1f}%")
                                    st.write(f"- Neutral (ι ≈ 0): {neutral_pct:.1f}%")
                ...
                with st.expander("🔧 Technical Details"):
                    ...
                    st.write(f"**Window Overlap**: {rolling_results['step_size_days']} day steps")
                    st.write(f"**Analysis Method**: Rolling window comparison against historical slices")
                    
                    # Show window details
                    if rolling_results.get('windows'):
                        first_window = rolling_results['windows'][0]
                        last_window = rolling_results['windows'][-1]
                        st.write(f"**First Window**: {first_window['start_date']} to {first_window['end_date']}")
                        #!/usr/bin/env python3
"""
Enhanced Streamlit Version of Iota Calculator with Rolling Window Analysis
Compatible with your existing sim.py and requirements.txt
