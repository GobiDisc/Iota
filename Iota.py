...
# ===== ROLLING ANALYSIS FUNCTIONS (From your original Iota.py) =====
...
- MINIMAL/LOW: Strategy performing consistently with backtest expectations
- MODERATE: Some performance degradation detected - monitor closely  
- HIGH: Significant degradation - strategy likely overfit to historical data
...
def create_csv_data(ar_stats, sh_stats, cr_stats, so_stats, 
    ...
    for metric_name, stats_dict, oos_val in metrics:
        ...
        if rolling_results and rolling_results.get('sufficient_data', False):
            ...
            # Add individual metric slopes
            metric_slopes = rolling_results.get('metric_slopes', {})
            metric_key = metric_name.lower().replace('_', '')[:2]  # 'sh', 'cr', 'so', 'an'
            if metric_key == 'an':  # Annualized return
                metric_key = 'ar'
            slope_key = f'{metric_key}_slope'
            if slope_key in metric_slopes:
                row[f'{metric_name}_Trend_Slope'] = metric_slopes[slope_key]
        else:
            row.update({
                'Overfitting_Risk': 'INSUFFICIENT_DATA',
                'Rolling_Windows': 0,
                ...
                'Trend_Slope': np.nan,
                'Degradation_Score': np.nan,
                f'{metric_name}_Trend_Slope': np.nan
            })
